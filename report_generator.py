...
def build_excel_report(result) -> bytes:
   ...
   cis_pass = sum(1 for c in result.cis if c["status"] == "PASS")
   cis_fail = sum(1 for c in result.cis if c["status"] == "FAIL")
   cis_unk  = sum(1 for c in result.cis if c["status"] == "UNKNOWN")
   total = len(result.cis) or 1
   compliance = round(100.0 * cis_pass / total, 2)
   dash["A6"]  = "CIS Controls (subset)"; dash["B6"]  = len(result.cis)
   dash["A7"]  = "CIS PASS";             dash["B7"]  = cis_pass
   dash["A8"]  = "CIS FAIL";             dash["B8"]  = cis_fail
   dash["A9"]  = "CIS UNKNOWN";          dash["B9"]  = cis_unk
   dash["A10"] = "Compliance % (PASS/Total)"; dash["B10"] = compliance
   dash["A12"] = "Permissive Rules (>=MEDIUM)"; dash["B12"] = len(result.permissive)
   dash["A13"] = "Duplicates";                   dash["B13"] = len(result.duplicates)
   dash["A14"] = "Shadowed";                     dash["B14"] = len(result.shadowed)
   dash["A15"] = "Redundant";                    dash["B15"] = len(result.redundant)
   dash["A17"] = "Internet UTM coverage %";      dash["B17"] = result.sec_profile_coverage.get("utm_coverage_pct", 0)
   # Lifecycle on dashboard
   life = result.lifecycle_assessment or {}
   dash["A19"] = "Firmware Lifecycle Status"; dash["B19"] = life.get("firmware_status", "Review")
   dash["A20"] = "Security Exposure";         dash["B20"] = life.get("security_exposure", "Unknown")
   for r in range(6, 21):
       dash[f"A{r}"].font = HFONT
       dash[f"A{r}"].alignment = WRAP
   autosize(dash, min_w=18, max_w=70)
   # ----------------------------
   # CIS Scorecard (full subset)
   # ----------------------------
   ...
   fail_rows = [
       ...
       for c in result.cis if c["status"] == "FAIL"
   ]
   if not fail_rows:
